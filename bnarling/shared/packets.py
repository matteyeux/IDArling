...
class Default(Serializable):
    ...
    def attrs(dct):
        ...
        """
        return {key: val for key, val in dct.items() if not key.startswith("_")}
...
class Packet(with_metaclass(PacketFactory, Serializable)):
    ...
    def __repr__(self):
        ...
        attrs = ["{}={}".format(k, v) for k, v in Default.attrs(self.__dict__).items()]
...
class EventFactory(PacketFactory):
    ...
    def __new__(mcs, name, bases, attrs):
        ...
        if cls.__event__ is not None and cls.__event__ not in EventFactory._EVENTS:

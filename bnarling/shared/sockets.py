...
class ClientSocket(QObject):
    ...
    def set_keep_alive(self, cnt, intvl, idle):
        ...
        if tcp_keepidle is not None:
            ...
            self._socket.ioctl(sio_keeplive_vals, (1, idle * 1000, intvl * 1000))
    ...
    def _notify_write(self):
        ...
        # Trigger the upback
        if isinstance(self._write_packet, Container) and self._write_packet.upback:
        ...
        if self._write_cursor >= len(self._write_buffer) and not self._outgoing:
            self._write_notifier.setEnabled(False)
